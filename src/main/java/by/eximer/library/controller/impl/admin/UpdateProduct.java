...
public class UpdateProduct implements Command {
	...
	private static final String STR_ACTIONS_PARAM_NAME = "str_actions";
	private static final String TIPY_PARAM_NAME = "tipy";
	...
	strActions = request.getParameter(STR_ACTIONS_PARAM_NAME);
	tipy = request.getParameter(TIPY_PARAM_NAME);
	name = request.getParameter(NAME_PARAM_NAME);
	rest = request.getParameter(REST_PARAM_NAME);
	bigText = request.getParameter(BIG_TEXT_PARAM_NAME);
	categories = request.getParameter(CATEGORIES_PARAM_NAME);
	...
	try {
		...
		user = userService.updateProduct(Integer.parseInt(sessionId), name, text, bigText, Integer.parseInt(idProduct), rest, categories, tipy);
		if (user.getSuccess()) {
